import { View, Button, Text, Modal, SafeAreaView, ActivityIndicator, FlatList, Alert } from 'react-native';
...
export default function ToDo({ navigation }) {
  ...
  let deleteToDo = async (toDoId) => {
    ...
    Alert.alert('Alert', 'Your Todo is Removed', [
  ...
  let renderToDoItem = ({item}) => {
    return (
      <View style={[AppStyles.rowContainer, AppStyles.rightMargin, AppStyles.leftMargin]}>
        ...
        <InlineTextButton text="Delete" color="#2c7853" onPress={() => deleteToDo(item.id)  } />
      </View>
    );
  }
  ...
  return (
    <SafeAreaView>
      <View style={[AppStyles.rowContainer, AppStyles.rightAligned, AppStyles.rightMargin, AppStyles.topMargin]}>
        <InlineTextButton text="Manage Account" color="#2c4e78" onPress={() => navigation.navigate("ManageAccount")}/>
      </View>
      <Modal
        animationType="slide"
        transparent={true}
      ...
      <Text style={AppStyles.header}>ToDo List</Text>
